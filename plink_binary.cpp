...
const char DEFAULT_BED_MODE = 1;

plink_binary::plink_binary(void)
{
	bed_mode = DEFAULT_BED_MODE;
}
...
void plink_binary::write_individual(const vector<int> genotypes) {
    if (genotypes.size() == 0) {
        throw gftools::malformed_data("No genotypes defined");
    }
    if (snps.size() == 0) {
        throw gftools::malformed_data("No SNPS defined");
    }
    if (genotypes.size() != snps.size()) {
        stringstream ss;
        ss << "Incorrect SNPs count: ";
        ss << genotypes.size();
        ss << " whereas ";
        ss << snps.size();
        ss << " SNPs defined";
...
void plink_binary::bed_write(vector<int> genotypes) {
	int len = (3 + snps.size()) / 4;
	char buffer[len];
	compress_calls(buffer, genotypes);
	bed_file->write(buffer, len);
}
